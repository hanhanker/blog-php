...
function WSTVerify(){
	ob_clean();
	$Verify = new \verify\Verify();  // 使用extend中的类
    $Verify->length   = 4;
    $Verify->entry();
}
/**
...
function WSTConf($key,$v = ''){
	...
	}else if($v === ''){ //不传递第二个参数则是这个操作
		if(array_key_exists('WSTMARTCONF',$GLOBALS)){
			$conf = $GLOBALS['WSTMARTCONF'];
			$ks = explode(".",$key); // ['CONF','wstVersion'] 'CONF'是标志.后面的变成一个数组放在CONF中
			for($i=0,$k=count($ks);$i<$k;$i++){//如果想取sys_configs表中的数据那么就是 WSTConf('CONF.aa')
				if(array_key_exists($ks[$i],$conf)){ // 第一次获取$GLOBALS['WSTMARTCONF']['CONF']的全部元素，第二次循环获取对应的值
					$conf = $conf[$ks[$i]];
				}else{
					return null;
				}
...
function WSTGetFirstCharter($str){
	...
	$fchar=ord($str{0}); //返回字符串第一个字符的 ASCII 码值。 
...
function WSTConfig(){
	...
	if(!$rs){
		...
		//获取上传文件目录配置
		$data = Db::name('datas')->where('catId',3)->column('dataVal');//[0=>'aaa',1=>'bbb'];
		foreach ($data as $key => $v){
			$data[$key] = str_replace('_','',$v);//把路径中的_删除
		}
		$rs['wstUploads'] = $data;//mallSlogan 商城口号：实用的多用户商城系统！  //Powered By WSTMart
